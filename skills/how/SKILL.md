...
After the explanation is complete, spawn two architectural critics concurrently. Use `claude-sonnet-5@max` and `gpt-5.6-sol@max` unless the user explicitly requests another panel.

For each critic:
- `agent_type`: `general-purpose`
- `model` and `reasoning_effort`: split one of the two specifications above. The lead may escalate effort when the selected model supports it.
- `mode`: `background`
- prompt: explicitly forbid file modifications
