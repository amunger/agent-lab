...
Use the arena defaults: `claude-sonnet-5@max`, `gpt-5.6-sol@max`, and `grok-4.6@high`. If the task tool rejects a model or effort, follow the arena skill's inline fallback rule. Do not look for a global pstack configuration.
...
The synthesis can ship as its own commit either way. Subsequent commits can fill in bodies against the stable contract. Planned and scoped breakage during fill-in is acceptable when the work converges directly on the agreed design. For adversarial pressure before implementing, run the **interrogate** skill on the synthesized sketch.
...
If implementation keeps producing friction the sketch cannot absorb, throw the sketch out. Do not bolt fixes onto a wrong design. Revisit the root cause and redesign from the requirement.
...
3. Remove obsolete or redundant parts before adding replacements. The new sketch should be smaller than the old one before it grows.
