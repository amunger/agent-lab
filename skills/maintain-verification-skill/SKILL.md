...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.github/skills/verify-*/`). Several candidates → ask which one with `ask_user`; none → stop and point at `/create-verification-skill` instead of inventing a target.
